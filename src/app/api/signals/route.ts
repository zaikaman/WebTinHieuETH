...
export async function GET(request: Request) {
  try {
    ...
    const updatePromises = result.rows
      ...
      .map(async signal => {
        // Check if status just changed from ACTIVE to STOPPED
        const previousStatus = await pool.query(
          `SELECT status FROM signals WHERE id = $1`,
          [signal.id]
        );
        
        const statusChanged = previousStatus.rows[0]?.status === 'ACTIVE';
        
        // Update signal profit
        await pool.query(
          `UPDATE signals SET profit = $1 WHERE id = $2 AND (profit IS NULL OR profit != $1)`,
          [Number(signal.profit), signal.id]
        );
        
        // Only update balance stats if status changed from ACTIVE to STOPPED
        if (statusChanged) {
